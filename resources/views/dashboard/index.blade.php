...
<div class="row g-4 mb-4">
    <div class="col-md-3"><div class="card p-3 border-start border-4 border-success"><h6 class="text-muted">Pendapatan Hari Ini</h6><h3 class="fw-bold text-success">Rp {{ number_format($stats['revenue'], 0, ',', '.') }}</h3></div></div>
    <div class="col-md-3"><div class="card p-3 border-start border-4 border-primary"><h6 class="text-muted">HP Terjual Hari Ini</h6><h3 class="fw-bold text-primary">{{ $stats['sold'] }} Unit</h3></div></div>
    <div class="col-md-3"><div class="card p-3 border-start border-4 border-warning"><h6 class="text-muted">Total Stok Ready</h6><h3 class="fw-bold text-warning">{{ $stats['stock'] }} Unit</h3></div></div>
    <div class="col-md-3"><div class="card p-3 border-start border-4 border-info"><h6 class="text-muted">Transaksi Hari Ini</h6><h3 class="fw-bold text-info">{{ $stats['txns'] }} Trx</h3></div></div>
</div>
<div class="row">
        ...
        <div class="card p-4">
            ...
            <table class="table table-hover" id="tblRecent">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><small>{{ $trx->transaction_code }}</small></td>
                        <td>Rp {{ number_format($trx->total_price, 0, ',', '.') }}</td>
                        <td><button class="btn btn-sm btn-outline-primary" onclick="showDetail({{ $trx->id }})"><i class="fas fa-eye"></i></button></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
<script>
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (Rp)', data: @json($chartData), borderColor: '#059669', backgroundColor: 'rgba(5, 150, 105, 0.1)', fill: true, tension: 0.4 }]
        },
